...
static void outCsr(unsigned char command){
  ...
  Wait(2160);                  // blind cycle 90 us Wait
}
...
// Be careful, TSCR1 and TSCR2 maybe set in other rituals
  ...
  outCsrNibble(0x03); // (DL=1 8-bit mode)
  Wait(2400);          // blind cycle 100us Wait
  outCsrNibble(0x03); // (DL=1 8-bit mode)
  Wait(2400);          //  blind cycle 100us Wait (not called for, but do it anyway)
  outCsrNibble(0x02); // (DL=0 4-bit mode)
  Wait(2400);          // blind cycle 100 us Wait
