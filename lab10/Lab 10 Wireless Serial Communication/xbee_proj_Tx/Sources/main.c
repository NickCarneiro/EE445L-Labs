...
//---------------------mainTx---------------------
// main used for transmitter
// Input: none
// Output: none
char string[75];  // global to assist in debugging  
unsigned short n;  
void main(void) {		
  ...
  SCIb_Init(9600);   //enables interrupts and init's fifo and interrupts
  XBee_Init();
  SCI1_OutString("TechArts DP512, SCI demo 8/10/09 -JWV"); OutCRLF();
  //SCIb_OutString("init test");
  while(1) {  
    //string = XBee_CreateTxFrame();
    SCI1_OutString("InString: "); 
    SCI1_InString(&string[0],75); //Typed Terminal String->string 
    //string[0] = SCI1_InChar();
    OutCRLF();
    while(!(XBee_SendTxFrame(XBee_CreateTxFrame(&string[0]))));
  }
}