...
/*   
  ...
  ground = pin 1    Vss
  power  = pin 2    Vdd   +5V
  pot in = pin 3    Vlc   grounded for highest contrast
  PT2    = pin 4    RS    (1 for data, 0 for control/status)
  ground = pin 5    R/W   (1 for read, 0 for write)
  PT1    = pin 6    E     (enable)
  PP6    = pin 14   DB7   (4-bit data)
  PP5    = pin 13   DB6
  PP4    = pin 12   DB5
  PP3    = pin 11   DB4
...
#define HUNDRED_MICROS 20     //160   ///20   //150
#define NINETY_MICROS 18    /// 145    //18   //135
#define ONESIXFOUR_MS 310     //2560      //310   //2460
#define ONE_MS 190      ///1600     ///190    //1500
#define CR 13
#define TAB 9
#define LF 10

static unsigned short OpenFlag = 0;
static unsigned short letter_cnt = 0;
...
static void outCsrNibble(unsigned char command){
  PTP = (PTP&~0x78)+((command&0x0F)<<3);    //nibble, E=0, RS=0
  PTT &= ~0x06;
  //PTT |= 0x04;
  PTT |= 0x02;             //E goes 0->1
  asm nop
...
unsigned short LCD_Init(void){
  ...
  DDRP |= 0x78;       //PH6-0 output to LCD
  DDRT |= 0x06;
  //PTT &= ~0x02;       //PH5=R/W=0 means write
  //PTT |= 0x04;
  TSCR1 = 0x80;       //Enable TCNT, 24MHz boot mode, 4MHz in run mode
  TSCR2 = 0x07;       //divide by 16 TCNT prescale, TCNT at 667nsec
...
unsigned short LCD_OutChar(unsigned char letter){
  ...
  PTP = (PTP&~0x78)+((0x0F&(letter>>4))<<3);  //ms nibble, E=0, RS=1
  PTT &= ~0x02;
  PTT |= 0x04;
  PTT |= 0x02;       //E goes 0->1
  asm nop
  asm nop            //5 cycles wide = 208ns
  PTT &= ~0x02;      //E goes 1->0
  PTP = (PTP&~0x78)+((letter&0x0F)<<3);  //ls nibble, E=0, RS=1
  PTT &= ~0x02;
  PTT |= 0x04;
  PTT |= 0x02;       //E goes 0->1
  asm nop
